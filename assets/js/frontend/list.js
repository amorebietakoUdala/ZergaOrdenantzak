...
$(document).ready(function () {
   ...
   $.getJSON(url, function (data) {
      ...
      $.each(data, function (i, ordenantza) {
         ...
         items.push('<li"><a data-toggle="tooltip" data-container="body" data-placement="top" title="Haz click aquí para ver el texto de la ordenanza" target="_blank"  href="/zergaordenantzak/api/ordenantza/' + ordenantza.id + '?_format=html">' + ordenantza.kodea_prod + ' - ' + (
         ...
         $.each(atalak, function (i1, atala) {
            if (atala.izenburuaes_prod !== "") {
               if (('kodea_prod' in atala) && ('izenburuaes_prod' in atala)) {
                  items.push('<li><ul class="nirul"><li class="nireli"><a target="_blank"  href="/zergaordenantzak/api/ordenantza/' + ordenantza.id + '?_format=html#' + ordenantza.id + '-' + atala.id + '">' + atala.kodea_prod + ' - ' + (
